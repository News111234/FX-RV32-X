...
```bash
cd python
python asm_to_hex.py ../uvm/load_use_test.s ../uvm/load_use_test.hex
python asm_to_hex.py ../uvm/intr_test.s    ../uvm/intr_test.hex
```
...
| 测试 | 命令 |
|------|------|
| Load-Use 冒险测试 | `set HEX_FILE load_use_test.hex; set TEST_NAME cpu_test_hazard` |
| 中断测试 | `set HEX_FILE intr_test.hex; set TEST_NAME cpu_test_interrupt` |
| 基础指令测试 | `set HEX_FILE alu_test.hex; set TEST_NAME cpu_test_alu` |
...
| `HEX_FILE` | `alu_test.hex` | 测试程序 hex 文件 |
| `GUI_MODE` | `0` | 1=打开波形窗口 |
...
├── load_use_test.s           # Load-Use 冒险测试汇编
├── load_use_test.hex         # Load-Use 冒险测试 hex（由 asm_to_hex.py 生成）
└── intr_test.s               # 中断测试汇编
```
