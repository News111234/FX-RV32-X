# run_cli_degradation.do — 降级复用测试 (BANKS=1, POL=1)
...
file copy -force degradation_test.hex nested_test.hex
vsim -onfinish stop -c -gUSE_INST_ROM=1 -gSHADOW_BANKS=1 -gOVERFLOW_POLICY=1 work.tb_nested_check
run 10us
set r [exam u_soc_top/u_data_ram/mem\[65\]]
puts "mem\[65\] = $r (expected 32'hBEEF0003)"
if {[string equal -nocase $r 32'hBEEF0003]} { puts "=> PASS (GPIO抢占成功, Bank覆盖)" } else { puts "=> FAIL ***" }
quit -f
