...
_start:
    ...
    sw zero, 0x120(x0)   # mem[72]  mcycle: sw ISR entry
    sw zero, 0x124(x0)   # mem[73]  mcycle: timer ISR entry
    sw zero, 0x128(x0)   # mem[74]  mcycle: gpio ISR entry
...
isr_sw:
    csrr t4, mcycle          # 记录 SW ISR 进入时间
    sw t4, 0x120(x0)         # → mem[72]
    li t0, 0xCAFE0003
...
isr_timer:
    csrr t4, mcycle          # 记录 Timer ISR 进入时间
    sw t4, 0x124(x0)         # → mem[73]
    li t0, 0xDEAD0007
...
isr_gpio:
    csrr t4, mcycle          # 记录 GPIO ISR 进入时间
    sw t4, 0x128(x0)         # → mem[74]
    li t0, 0xBEEF000B
