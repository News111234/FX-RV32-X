# run_cli_context.do — 寄存器完整性测试 (BANKS=4)
...
file copy -force context_integrity_test.hex nested_test.hex
vsim -onfinish stop -c -gUSE_INST_ROM=1 -gSHADOW_BANKS=4 work.tb_nested_check
run 10us
set r [exam u_soc_top/u_data_ram/mem\[63\]]
puts "tohost = $r (expected 32'h00000000)"
if {[string equal -nocase $r 32'h00000000]} { puts "=> PASS" } else { puts "=> FAIL ***" }
quit -f
